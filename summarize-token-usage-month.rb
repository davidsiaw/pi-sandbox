...
# summarize-token-usage-month.rb -- per-model summary of one MONTH of
# pa-token-usage rows, plus a day-by-day trend.
#
#   ruby summarize-token-usage-month.rb              # most recent month on file
#   ruby summarize-token-usage-month.rb 2026-08      # a specific month
#
# Companion to summarize-token-usage.rb, which does the same for a single day.
# Same host tool, same data dir, same columns -- it just folds every
# YYYY-MM-*.csv in the month into one set of groups.
#
# Like its sibling this is a HOST tool and is not baked into the image: it lives
# at the repo root, which no COPY in the Dockerfile touches.
...
if ARGV.any? { |a| %w[-h --help help].include?(a) }
  puts <<~USAGE
    summarize-token-usage-month.rb -- per-model summary of one month of rows.

    Usage:
      ruby summarize-token-usage-month.rb              most recent month on file
      ruby summarize-token-usage-month.rb 2026-08      a specific month
      ruby summarize-token-usage-month.rb --help       this message

    Reading from:
      #{DATA_DIR}
      (override with PI_CODING_AGENT_DIR)

    Prints two tables: totals per model for the whole month, and one row per
    day so you can see where the spend actually went.
...
# YYYY-MM names sort lexicographically in date order, so a plain max over the
# month prefixes is the newest month with data -- not necessarily this month.
def latest_month(files)
  files.map { |f| File.basename(f)[0, 7] }.max
end
...
arg = ARGV[0]
abort "expected a month like 2026-08, got: #{arg}" if arg && arg !~ /\A\d{4}-\d{2}\z/

month = arg || latest_month(files)
month_files = files.select { |f| File.basename(f).start_with?("#{month}-") }.sort
abort "no logs for #{month} in #{DATA_DIR}" if month_files.empty?
...
# than dying on the whole month.
month_files.each do |path|
  day = File.basename(path, ".csv")
  begin
...
abort "no rows for #{month}" if by_model.empty?
...
puts "#{month}  (#{days_with_data} day#{'s' unless days_with_data == 1} with data, #{DATA_DIR})"
puts
...
puts
day_body = by_day.sort_by { |day, _| day }.map { |day, g| to_row(day, g) }
