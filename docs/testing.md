...
| system default resolves to a 3.4.x | the pin actually resolves, not just parses. Run under `MISE_OFFLINE=1` on purpose: expanding the partial pin `"3.4"` otherwise needs the ruby-build definition list, which mise caches in the **ephemeral** home and so refetches every container — and when GitHub rate-limits that fetch, `mise current ruby` prints an empty stdout with exit 0 and the check fails for reasons unrelated to the image. Offline it answers from config alone (`3.4` cold, `3.4.7` warm), which is all this test needs |
...
| pa-shaped run: session written into the bind-mounted project dir | `--session-dir "$PWD/.pi-sessions"` inside a project mounted at its real host path actually produces a `.jsonl`, which is what makes sessions survive the container. Fixture gotcha: the project dir must be `a+rwX`, not just `a+rX` — pi's `SessionManager` does an unguarded `mkdirSync` at startup and dies with `EACCES` before building a single request, which shows up as *both* `HDR_AUTH=nocapture` (the credential-chain check above) and `SESSION_MISSING`. Real `pa` never hits it: it runs the container as the host uid, which owns the project. On failure the run now echoes `PI_STDERR=…` so a startup crash isn't misread as a credential bug |
