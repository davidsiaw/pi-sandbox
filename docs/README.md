...
- [console-repl.md](console-repl.md) — the `page_console` / `page_screenshot` / `page_close` tools: one **live** page an agent can send JS to repeatedly and read the console from, so "click this, then look" is expressible; the single chronological stream that mixes agent logging with page errors and 5xx responses, why late errors are never missed, and why an idle browser is worth closing
...
- [checker.md](checker.md) — the `pa-checker` extension: an opt-in second model that audits an answer before you see it, wired to a `"checker"` key in `models.json`; what the auditor is shown (system prompt, request, tool log, answer), why it gets a read-only tool set enforced three ways, why the revision loop is bounded and always fails open, and why the criticism can only come back as a user-role message
